 ...
 */
export function extractDataFromText(text: string): ParsedResume {
...
function extractPersonalInfo(text: string, lines: string[]) {
    ...
    for (let i = 0; i < Math.min(5, lines.length); i++) {
        ...
        if (line.match(/@|phone|email|linkedin|github|resum|cv|curriculum|profile/i)) continue;
...
function extractEducation(text: string, lines: string[]) {
    ...
    const eduStartIdx = lines.findIndex((line) =>
        /^(education|academic\s+background)/i.test(line)
    );
    if (eduStartIdx === -1) return education;
    ...
    const isSchool = (l: string) => l.match(/(University|College|Polytechnic|School|Academy|Institute of Technology|Sreenidhi Institute)/i);
    const isDegree = (l: string) => l.match(/(Bachelor|Master|PhD|B\.S\.|M\.S\.|B\.A\.|M\.A\.|MBA|B\.Tech|M\.Tech)/i);
 ...
 */
function extractExperience(text: string, lines: string[]) {
    const experience: ParsedResume["experience"] = [];

    const expStartIdx = lines.findIndex((line) =>
        /^(professional\s+experience|experience|employment|work\s+history|work\s+experience)/i.test(line)
    );
    if (expStartIdx === -1) return experience;
...
function extractSkills(text: string, lines: string[]) {
    ...
    const skillsStartIdx = lines.findIndex((line) =>
        /^(skills|technical\s+skills|technologies|core\s+competencies)/i.test(line)
    );
    if (skillsStartIdx === -1) return skills;
 ...
 * Extract projects
 */
function extractProjects(text: string, lines: string[]) {
    const projects: ParsedResume["projects"] = [];

    const projStartIdx = lines.findIndex((line) => /^projects/i.test(line));
    if (projStartIdx === -1) return undefined;
 ...
 */
function extractCertifications(text: string, lines: string[]) {
    const certStartIdx = lines.findIndex((line) =>
        /^certifications/i.test(line)
    );
    if (certStartIdx === -1) return undefined;
